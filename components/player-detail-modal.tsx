...
export function TopPlayers() {
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        ...
        const sortedPlayers = allPlayers.sort((a, b) => b.overallPoints - a.overallPoints)
  ...
  const getPlayerColor = (name: string) => {
    if (name.toLowerCase() === "smp") return "text-red-600"
    if (name.toLowerCase() === "nethpot") return "text-[#5a0000]"
                ...
                <div className="flex items-center gap-3 sm:gap-4 md:gap-6">
                  ...
                  <div className="flex-1 flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className={`font-black text-sm sm:text-base md:text-lg uppercase truncate ${getPlayerColor(player.name)}`}>
                        {player.name}
                      </h3>
                      <p className="text-xs text-muted-foreground truncate">Tested</p>
                    </div>
                  </div>
